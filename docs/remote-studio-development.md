...
- On `tertiary`, the committed remote screenshot lane now has a second-stage GUI-session relay through the logged-in `Terminal` desktop session. When that relay is active, the sidecar should report `guiSessionRelay.method="terminal.command"` and should preserve the inner `studio_ui_control.py capture-screenshot` method (`window`, `rect`, or `display`). Treat only `window` or `rect` as authoritative visual proof; whole-display fallback is diagnostic evidence, not parity proof.
