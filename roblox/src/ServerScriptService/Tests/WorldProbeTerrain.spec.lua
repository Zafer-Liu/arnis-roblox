return function()
    ...
    Assert.equal(flatSummary.edgeMeanTerrainY, 12, "expected flat terrain to preserve the perimeter mean height")
    ...
    Assert.equal(
        flatSummary.centerMinusEdgeMeanStuds,
        0,
        "expected flat terrain not to invent a center-versus-edge height bias"
    )
    ...
    Assert.equal(steppedSummary.edgeMeanTerrainY, 12, "expected stepped terrain to preserve the rounded perimeter mean")
    ...
    Assert.equal(
        steppedSummary.centerMinusEdgeMeanStuds,
        0,
        "expected stepped terrain to report when the center and perimeter stay roughly aligned"
    )
    ...
    Assert.equal(
        sparseSummary.edgeMeanTerrainY,
        nil,
        "expected sparse terrain summary not to invent a perimeter mean when edge samples are missing"
    )
    Assert.equal(
        sparseSummary.centerMinusEdgeMeanStuds,
        nil,
        "expected sparse terrain summary not to invent a center-edge bias without enough data"
    )
    ...
    Assert.equal(fullSummary.edgeMeanTerrainY, 10, "expected full edge coverage to preserve the perimeter mean")
    Assert.equal(fullSummary.edgeTerrainYRangeStuds, 8, "expected edge range to capture the perimeter cliff")
    Assert.equal(
        fullSummary.centerMinusEdgeMeanStuds,
        0,
        "expected full edge coverage to report no center peak when center and perimeter averages match"
    )
    Assert.equal(
    ...
    Assert.equal(
        sparseEdgeSummary.edgeMeanTerrainY,
        10.7,
        "expected sparse edge coverage to preserve the rounded perimeter mean from the remaining samples"
    )
    Assert.equal(
        sparseEdgeSummary.centerMinusEdgeMeanStuds,
        1.3,
        "expected sparse edge coverage to surface a center-versus-edge bias from the remaining samples"
    )
    Assert.equal(
    ...
    local centerPeakSummary = WorldProbeTerrain.summarizeTerrainSamples({
        { terrainY = 10.0, terrainMaterial = "Grass" },
        { terrainY = 10.0, terrainMaterial = "Grass" },
        { terrainY = 18.0, terrainMaterial = "Rock" },
        { terrainY = 10.0, terrainMaterial = "Grass" },
        { terrainY = 10.0, terrainMaterial = "Grass" },
    }, {
        centerIndex = 3,
    ...
    Assert.equal(centerPeakSummary.edgeMeanTerrainY, 10, "expected center-peak terrain to preserve the edge mean")
    Assert.equal(
        centerPeakSummary.centerMinusEdgeMeanStuds,
        8,
        "expected center-peak terrain to expose the center-versus-edge height bias directly"
    )
end
