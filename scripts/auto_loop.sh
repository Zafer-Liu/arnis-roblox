...
if [[ "$SKIP_HARNESS" -eq 1 ]]; then
  ...
  echo "[dry-run] bash scripts/run_studio_harness_remote.sh -- --small-place --play-wait 120 --takeover"
  PHASE_RESULT_HARNESS="dry"
else
  if bash scripts/run_studio_harness_remote.sh -- --small-place --play-wait 120 --takeover; then
