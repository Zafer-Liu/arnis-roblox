...
class RunStudioHarnessRemoteTests(unittest.TestCase):
    ...
    def test_runs_same_remote_harness_with_remote_vsync_binary(self) -> None:
        ...
        self.assertIn('pkill -f "$remote_arnis_dir/scripts/run_studio_harness.sh" || true', self.text)
        self.assertIn('pkill -f "$remote_vsync_target_dir/debug/vsync serve" || true', self.text)
