...
class AustinFidelityScriptTests(unittest.TestCase):
    ...
    def test_export_to_lua_documents_bounded_dev_profile_default(self) -> None:
        ...
        self.assertIn('--chunks-per-shard "$RUNTIME_CHUNKS_PER_SHARD"', text)
        self.assertIn('runtime_args=(', text)
        self.assertIn('--chunks-per-shard "$RUNTIME_CHUNKS_PER_SHARD"', text)
        self.assertIn('if [[ "$RUNTIME_SHARD_MAX_BYTES" -gt 0 ]]; then', text)
        self.assertIn('runtime_args+=(--max-bytes "$RUNTIME_SHARD_MAX_BYTES")', text)
